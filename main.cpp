...
int main(int argc, char *argv[])
{
    ...
    ////////////////////////////////////////////////
    // set up a loop
    ConstantNode *c1 = new ConstantNode( QVector3D(0, 5, 0), "ConstNode1" );
    a.addNode( c1 );
    c1->setConstValue( DataValue(1) );

    MergerNode *m = new MergerNode( QVector3D(0, 3, 0), "MergerNode" );
    a.addNode( m );
    ...
    ConstantNode *c2 = new ConstantNode( QVector3D(-4, 3, 0), "ConstNode2" );
    a.addNode( c2 );
    ...
    SplitterNode *s = new SplitterNode( QVector3D(4, 3, 0), "SplitterNode" );
    a.addNode( s );
    s->setNumOutputs(2);
    DataPort::connect(d->getOutput(0), s->getInput(0));
    DataPort::connect(s->getOutput(0), m->getInput(0));

    StdOutNode *o = new StdOutNode( QVector3D(0, -5, 0), "StdOutNode" );
    a.addNode( o );
